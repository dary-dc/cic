...
const Carousel = ({ selectedTemplate, onTemplateSelection, cards }) => {
  ...
  const handlePrev = () => {
    ...
    console.log("extendedCards[newIndex]", extendedCards[newIndex])
    let usableExtendedCards = extendedCards.length ? extendedCards : extendedCardsRef.current
    onTemplateSelection(usableExtendedCards[newIndex])
	}, [extendedCards, onTemplateSelection, cards.length])
  ...
  useEffect(() => {
    ...
    extendedCardsRef.current = newExtendedCards
  ...
  useEffect(() => {
    ...
    if (selectedTemplate?.id !== extendedCards.map(({ id }) => id)[currentIndex]) {
      ...
      // } else {
      //   updateIndex(newIndex + 1 + newCount);
      // }
      const newIndex = cards.findIndex(card => card.id === selectedTemplate?.id);
      if (newIndex !== -1) {
        updateIndex(newIndex + newCount);
      } else {
        if (cards.length === 0) {
	...
	}, [selectedTemplate, cards, updateIndex, currentIndex, extendedCards])
